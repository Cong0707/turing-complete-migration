...
`spec.isa` 只决定汇编指令如何编码，不决定游戏中 Program/RAM 元件可容纳多少字节。
如果汇编程序大于 Program 元件的 `buffer_size`，高地址会回绕到较低地址，现象很容易被
误判为某条指令编码错误。

实际诊断案例中，RV64 的 Program 元件容量为 4096 字节，而 `program.asm` 含 1825 条
32 位指令，共 7300 字节。源文件第 1099 行：
...
4096 字节 Program 只保留低 12 位地址，因此 `0x10bc` 回绕为 `0x00bc`。地址
`0x00bc` 恰好保存：
...
所以 CPU 读到 `0x00008067` 并不表示 LUI 被汇编成了 JALR，而是程序已经超过存储容量。
排查时应同时记录“源代码行、汇编后的字节地址、Program 容量、回绕后地址”，不能只比较
源代码行和 CPU 当前读数。

对于当前 7300 字节程序，应把 Program 容量调到至少 8192 字节；建议使用 16384 字节，
并确认地址链路能传递相应的高地址位。修改正在使用的存档前应先退出游戏，避免运行时
自动保存覆盖离线修改。
...
7. `lui x15, 0x76543` 的输出为 `b7 37 54 76`，对应机器码字
   `0x765437b7`；同时覆盖零值、ABI 寄存器别名和 20 位最大值边界。
