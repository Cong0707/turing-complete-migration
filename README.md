...
处理 v6 的旧元件枚举，再写出可以由 2.1.276 直接读取的完整 v15 电路容器。

## 当前能力

- 输入电路版本：v6、v7、v9、v10、v15。
- 输出电路版本：统一为 v15。
- 保留元件数、导线数、位置、旋转、永久 ID、位宽、Custom ID、Program 选择、
  linked components 及已识别设置。
...
- 0.1059：92/92 个主电路转换成功，1484 个元件、5330 条导线，转换前后逐文件计数一致。
...
- 自定义元件：34 个 foundry 定义覆盖 33 个被引用 ID 和 135 个实例，缺失/重复 ID 为 0。
- 自动化测试：26 项，测试数据均由代码生成，不含用户存档。
